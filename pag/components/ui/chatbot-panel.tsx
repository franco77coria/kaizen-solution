...
/**
 * Panel del chat. Vive en su propio chunk: lo carga chatbot.tsx con next/dynamic
 * recién cuando el usuario abre el chat, así framer-motion no entra en el bundle
 * inicial de la landing. Una vez montado se queda montado, para no perder la
 * conversación al cerrar y volver a abrir.
 */
export default function ChatPanel({ isOpen, onClose }: ChatPanelProps) {
    ...
    // Escape cierra el chat: en mobile ocupa toda la pantalla y sin esto la
    // única salida es encontrar la X.
    useEffect(() => {
        if (!isOpen) return
    ...
    return (
        <>
            {/* Chat Window
                El panel queda siempre montado para no perder la conversación al
                cerrar y volver a abrir, así que no usa AnimatePresence: se anima
                con `animate` y, cuando está cerrado, se vuelve inerte
                (visibility hidden + aria-hidden) para que no quede un diálogo
                invisible alcanzable con Tab o por un lector de pantalla. */}
            <div
                role="dialog"
                aria-modal="true"
                aria-label="Chat con KaiBot"
                aria-hidden={!isOpen}
                className={`fixed z-50
                            ...
                            motion-reduce:transition-none ${
                                isOpen
                                    ...
                                    : 'opacity-0 translate-y-5 scale-95 invisible pointer-events-none'
                            }`}
            >
                        <div className="relative h-full sm:rounded-2xl bg-white shadow-2xl shadow-gray-200/50 flex flex-col overflow-hidden border border-gray-100 sm:border-gray-200">
                            ...
                            <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-100 bg-white safe-area-top">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[11px] text-outer-space">Asistente de Kaizen Solution</p>
                                    </div>
                                </div>
                                <button
                                    onClick={onClose}
                                    className="text-outer-space hover:text-egyptian hover:bg-gray-50 rounded-lg h-11 w-11 flex items-center justify-center transition-colors"
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50 scrollbar-thin">
                                {messages.map((message, index) => (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <p className={`text-[11px] mt-1 ${message.role === 'user' ? 'text-white/70' : 'text-outer-space/70'}`}>
                                ...
                                {showQuickActions && messages.length === 1 && (
                                    ...
                                    >
                                        <p className="text-[11px] text-outer-space font-medium">Acciones rápidas:</p>
                                ...
                                {showLeadForm && (
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p className="text-[11px] text-outer-space mt-1">
                                        ...
                                        <div className="space-y-2">
                                            <div className="relative">
                                                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-outer-space/60">
                                                    <UserIcon />
                                                </div>
                                                <input
                                                    type="text"
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                ...
                                                className="text-[13px] text-outer-space hover:text-egyptian px-4 py-2.5 rounded-xl hover:bg-gray-50 transition-colors"
                                            >
                                                Después
                                            </button>
                                        </div>
                            ...
                            <div className="p-3 border-t border-gray-100 bg-white safe-area-bottom">
                                <div className="flex items-end gap-2">
                                    ...
                                    <button
                                        ...
                                        className="h-11 w-11 rounded-xl bg-[#0a0f1e] text-white hover:bg-egyptian transition-all disabled:opacity-30 flex items-center justify-center flex-shrink-0"
