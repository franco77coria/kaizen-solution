...
// ============================================
  ...
  function readOpsData(sheetId, sheetName) {
    try {
      const ss = SpreadsheetApp.openById(sheetId);
      let sheet = ss.getSheetByName(sheetName);
      
      if (!sheet) {
        console.warn(`Hoja '${sheetName}' no encontrada en spreadsheet ${sheetId}.`);
        return [];
      }
      ...
      // Iterar desde 1 asumiendo encabezado en 0
      for (let i = 1; i < data.length; i++) {
        const nombre = String(data[i][0] || '').trim().toUpperCase();
        const cc = String(data[i][1] || '').trim();
        const celular = String(data[i][6] || '').trim();
        
        if (nombre && cc) { // Tiene que tener nombre y cédula para ser válido
          records.push({
            NOMBRE: nombre,
            CC: cc,
            CELULAR: celular
      ...
      console.error(`Error leyendo hoja OPS:`, e);
      return [];
    }
  }

  function getRawDataCached() {
    const cacheKey = 'rawData_v6_ops';
    ...
    try {
      ...
      // ========== FUENTE 3: Hoja OPS (Base dash) ==========
      let recordsOps = [];
      try {
        recordsOps = readOpsData(CONFIG.SHEET2_ID, CONFIG.OPS_SHEET_NAME);
        console.log('✅ Base OPS:', recordsOps.length, 'registros');
      } catch (error) {
        console.error('⚠️ Error cargando Base OPS:', error);
      }
      ...
      const result = { success: true, data: allRecords, opsData: recordsOps };
      ...
      return { success: false, error: error.toString(), data: [], opsData: [] };
    }
  }
...
function getAllData() {
  try {
    ...
    const statsData = calculateStats(allRecords, rawDataResult.opsData);
...
function getStats() {
  ...
  const stats = calculateStats(rawDataResult.data, rawDataResult.opsData);
...
function calculateStats(records, opsRecords = []) {
  ...
  // Calcular aliados por OPS
  const opsStats = {};
  const metaConstante = 50;
  let totalMetaOps = 0;
  
  opsRecords.forEach(ops => {
    opsStats[ops.CC] = {
      nombre: ops.NOMBRE,
      cc: ops.CC,
      celular: ops.CELULAR,
  ...
  records.forEach(record => {
    ...
    // Identificar si el líder coincide con un OPS
    if (record.CC_LIDER) {
      const ccLider = String(record.CC_LIDER).trim();
      if (opsStats[ccLider] && record.NUMERO_DOCUMENTO) {
        const docReferido = String(record.NUMERO_DOCUMENTO).trim();
        if (docReferido && docReferido !== '' && docReferido !== '0' && docReferido.toLowerCase() !== 'null') {
          opsStats[ccLider].aliadosUnicos.add(docReferido);
        }
      }
    }
  ...
  // Calcular progresos y array final OPS
  let totalAliadosOps = 0;
  const opsDataArray = Object.values(opsStats).map(ops => {
    ops.aliados = ops.aliadosUnicos.size;
    ops.progreso = Math.min(100, (ops.aliados / ops.meta) * 100).toFixed(2);
    totalAliadosOps += ops.aliados;
    delete ops.aliadosUnicos;
    return ops;
  });
  const porcentajeAliadosOps = totalAliadosConDocumento > 0 ? ((totalAliadosOps / totalAliadosConDocumento) * 100).toFixed(2) : 0;
  const progresoMetaTotal = totalMetaOps > 0 ? ((totalAliadosOps / totalMetaOps) * 100).toFixed(2) : 0;
  ...
  return {
    ...
    // Métricas OPS
    totalMetaOps: totalMetaOps,
    totalAliadosOps: totalAliadosOps,
    progresoMetaTotal: progresoMetaTotal,
    porcentajeAliadosOps: porcentajeAliadosOps,
    opsData: opsDataArray.sort((a, b) => b.aliados - a.aliados) // Ordenado por aliados desc
  };
}
